...
- 顶部通过“用例录制”“用例回放”“语义导航”和“资产”在同一窗口内切换工作区。
...
四个工作区共享当前设备和同一套用例数据，切换工作区不会打开第二个主窗口。数据的实际位置取决于安装版、便携版或源码运行模式，见“数据目录与迁移”。
...
scrcpy 投屏固定在工作台左侧，用例录制、回放、语义导航和资产四个工作区共用。选设备后点「投屏录制」或「接入投屏」。需 scrcpy 2.4+。
