...
MaaQQLogin 是基于 [MaaFramework](https://github.com/MaaXYZ/MaaFramework) 的 Android ADB 自动化用例工作台，面向 QQ App 等移动端场景。它在一个窗口中完成设备截图、用例录制、用例管理、队列回放和命名区域的语义导航。
...
- “语义导航”页扫描真机的资源 ID、无障碍描述和文字区域，可给区域命名并直接执行自然语言点击命令。
...
执行时会锁定设备、录制页、用例库和队列编辑，避免运行中的配置漂移。失败自动重试最多执行一次；失败截图写入 `logs/failures/`。
...
2. 从候选区域中选择控件，填写区域名称和可选别名；需要跨页面导航时填写“点击后页面”。
3. “点击区域”用于执行命令，“页面标志”只用于判断当前页面。保存结果写入 `jobs/semantic_map.json`。
4. 输入“点击搜索”“打开设置”等命令并选择“添加为用例步骤”。生成结果可直接保存到用例库、转到“用例录制”继续编辑，或在设备上预览。

生成命令会根据“点击后页面”关系加入跨页面路径，并转换为按短边 720 归一化的 `DirectHit` 步骤。设备预览仍优先使用 Android `resource-id`、`content-desc` 和文字，界面结构匹配不到时才使用按屏幕比例保存的坐标。不同页面存在同名区域时，可在命令中加入页面名，或让当前页面识别结果自动消歧。
...
- 支持真机控件扫描、区域命名、别名匹配和跨页面语义点击。
- 支持删除单个用例；分类管理和拖动迁移暂未在当前 Qt 工作台中提供。
- 打包成功后自动清理临时目录，并更新 `dist/QQJobEditor/` 中的完整可运行目录。
- 更新本地运行目录时保留已有用例，以及这些用例引用的模板文件。
...
打包脚本会先校验 ProjectInterface 和 Pipeline Schema，再运行单元测试，最后使用 PyInstaller 生成发布 ZIP，并更新唯一的本地运行目录 `dist/QQJobEditor/`。构建使用的临时目录会在成功后自动删除，同时保留本地用例和用例引用的模板：
